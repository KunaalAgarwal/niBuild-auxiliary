...
- `workflows/fmri_flanker.cwl` — Main workflow file
- `workflows/fmri_flanker_job.yml` — Job file with pre-configured parameters
...
```bash
unzip fmri_flanker.crate.zip -d my_workflow
cd my_workflow
...
Open `workflows/fmri_flanker_job.yml` and replace file path placeholders with
...
unzip fmri_flanker.crate.zip -d my_workflow
cd my_workflow
chmod +x workflows/fmri_flanker.cwl run.sh run_singularity.sh prefetch_images.sh prefetch_images_singularity.sh
```

### 2. Edit the Job File

Open `workflows/fmri_flanker_job.yml` and replace file path placeholders with your actual data paths.

### 3. Run

```bash
cwltool workflows/fmri_flanker.cwl workflows/fmri_flanker_job.yml
```

With a specific output directory:

```bash
cwltool --outdir ./results workflows/fmri_flanker.cwl workflows/fmri_flanker_job.yml
```
...
cwltool --singularity --outdir ./results \
  ...
  workflows/fmri_flanker_job.yml
```
...
### Validation errors
Validate the workflow: `cwltool --validate workflows/fmri_flanker.cwl`
...
python3 resolve_bids.py \
  ...
  --job workflows/fmri_flanker_job.yml \
  --output job.yml \
  --relative-to .
cwltool workflows/fmri_flanker.cwl job.yml
```
...
### Manual Override
You can edit `workflows/fmri_flanker_job.yml` directly to specify custom file paths
