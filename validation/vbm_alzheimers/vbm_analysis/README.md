...
- `workflows/vbm_analysis.cwl` — Main workflow file
- `workflows/vbm_analysis_job.yml` — Job file with pre-configured parameters
...
- `applywarp_mask` — File
- `fslmaths_add_file` — File
- `fslmaths_sub_file` — File
- `fslmaths_div_file` — File
- `fslmaths_mas` — File
- `fslmaths_max_file` — File
- `fslmaths_min_file` — File
- `randomise_design_mat` — File
- `randomise_tcon` — File
- `randomise_fcon` — File
- `randomise_mask` — File
- `randomise_x_block_labels` — File
...
```bash
unzip vbm_analysis.crate.zip -d my_workflow
cd my_workflow
...
Open `workflows/vbm_analysis_job.yml` and replace file path placeholders with
...
unzip vbm_analysis.crate.zip -d my_workflow
cd my_workflow
chmod +x workflows/vbm_analysis.cwl run.sh run_singularity.sh prefetch_images.sh prefetch_images_singularity.sh
```

### 2. Edit the Job File

Open `workflows/vbm_analysis_job.yml` and replace file path placeholders with your actual data paths.

### 3. Run

```bash
cwltool workflows/vbm_analysis.cwl workflows/vbm_analysis_job.yml
```

With a specific output directory:

```bash
cwltool --outdir ./results workflows/vbm_analysis.cwl workflows/vbm_analysis_job.yml
```
...
cwltool --singularity --outdir ./results \
  ...
  workflows/vbm_analysis_job.yml
```
...
### Validation errors
Validate the workflow: `cwltool --validate workflows/vbm_analysis.cwl`
