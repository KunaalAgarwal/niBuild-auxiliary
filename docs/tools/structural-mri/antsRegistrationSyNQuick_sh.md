...
**Typical Use:** Quick registration when speed is priority over maximum accuracy.

## Key Parameters

-d (dimension), -f (fixed), -m (moving), -o (output prefix), -t (transform type)

## Key Points

Same interface as antsRegistrationSyN.sh but ~4x faster with slightly less accuracy.
...
| `log` | `File` | `antsRegistrationSyNQuick.log` |
