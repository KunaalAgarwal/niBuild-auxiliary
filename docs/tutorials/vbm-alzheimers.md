...
- Generates FSL design matrix (`design.mat`) and contrast (`design.con`) files
    -   Relevant if you are running group-level significance tests following preprocessing

**Resulting cohort:** 235 subjects: 135 controls (CDR = 0) and 100 Alzheimer's (CDR >= 0.5: 70 very mild, 28 mild, 2 moderate). Mean age: 72.3 years (controls: 69.1, Alzheimer's: 76.8). Covariates are age and sex.
...
This tutorial demonstrates niBuild's direct file input approach which is useful for older datasets that are not organized in BIDS format.
...
In addition to the T1w data input, the pipeline needs several external files. Drag **Input** nodes from the I/O section for each:
...
## Step 5: Add Registration to Standard Space
